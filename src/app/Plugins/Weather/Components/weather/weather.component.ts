...
export class WeatherComponent implements OnInit {
    ...
    private getDateFromZonedDateTime(stamp: string | number, timezone?: string) {
        if (typeof stamp === 'number') {
            return moment(stamp * 1000).tz(timezone);
        }
    ...
    convertDateTime(stamp: string | number, timezone?: string): string {
        return this.getDateFromZonedDateTime(stamp, timezone).format('dddd, MMMM DD');
    }
}
